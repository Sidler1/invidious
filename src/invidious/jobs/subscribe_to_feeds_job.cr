class Invidious::Jobs::SubscribeToFeedsJob < Invidious::Jobs::BaseJob
  ...
  def begin
    ...
    loop do
      begin
        ucids = db.query_all("SELECT id FROM channels WHERE CURRENT_TIMESTAMP - subscribed > interval '4 days' OR subscribed IS NULL", as: String)
        ucids.each do |ucid|
        ...
        LOGGER.error("SubscribeToFeedsJob: #{ex.message}")
      end
