class Invidious::Jobs::RefreshFeedsJob < Invidious::Jobs::BaseJob
  ...
  def begin
    ...
    loop do
      begin
        emails = db.query_all("SELECT email FROM users WHERE feed_needs_update = true OR feed_needs_update IS NULL", as: String)
        ...
        LOGGER.error("RefreshFeedsJob: #{ex.message}")
      end
